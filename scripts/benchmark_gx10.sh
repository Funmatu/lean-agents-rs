#!/usr/bin/env bash
# scripts/benchmark_gx10.sh — Multi-model benchmark for lean-agents-rs on GX10 (UMA 128GB)
...
mkdir -p "$LOG_DIR"
REPORT_FILE="${LOG_DIR}/benchmark_gx10_report.md"
...
MODELS=(
    # 1. 超巨大モデルの極限量子化 (約89GB消費。残り39GBをコンテキストに回すため 16384 を指定)
    "Llama_397B_IQ1|llama|https://huggingface.co/ubergarm/Qwen3.5-397B-A17B-GGUF/resolve/main/Qwen3.5-397B-A17B-smol-IQ1_KT.gguf|Qwen3.5-397B-A17B-smol-IQ1_KT.gguf|16384"
    
    # 2. 中規模モデルの無劣化・爆速稼働 (フル精度。128GB環境なら 65536 の超長文コンテキストも余裕で動作)
    "SGLang_27B|sglang|Qwen/Qwen3.5-27B|0.90|65536"
)
...
if [[ ! -f "$REPORT_FILE" ]]; then
    echo "# GX10 (UMA 128GB) Benchmark Results" > "$REPORT_FILE"
    echo "" >> "$REPORT_FILE"
...
for MODEL_ENTRY in "${MODELS[@]}"; do
    ...
    if [[ "$ENGINE" == "llama" ]]; then
          ...
          - |
            ...
            MODEL_ID="$URL_OR_ID"
            MEM_FRAC="$ARG1"
            CTX_SIZE="$ARG2" # SGLangでもCtxSizeを受け取れるように改修
            
            # json-model-override-args と context-length を追加
            ENTRYPOINT_BLOCK="    command: >
          python3 -m sglang.launch_server --model-path ${MODEL_ID} --port 30000 --host 0.0.0.0 --trust-remote-code --mem-fraction-static ${MEM_FRAC} --context-length ${CTX_SIZE} --json-model-override-args '{\"chat_template_kwargs\": {\"enable_thinking\": false}}'"
...
echo ""
echo "=================================================="
echo " All GX10 benchmarks completed!"
